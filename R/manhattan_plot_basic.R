...
#' @param color_chr A vector specifying the colors to be used for different chromosomes.
...
#' @examples
#' # Example usage:
#' manhattan_plot_basic(data, pos_markers = "Marker_Position", y = "LogPval",
#'                     chr_col = "Chromosome", color_chr = c("gray", "darkgray"),
#'                     save_time = TRUE, GWAS_name = NULL, discard_chr = 11,
#'                     MAF_filter = 0.05, MAF_col = "MAF")
manhattan_plot_basic <- function(data, pos_markers = "Marker_Position", name_markers = "Marker_Name", y = "LogPval", chr_col = "Chromosome", color_chr = c("gray", "darkgray"), save_time = TRUE, GWAS_name = NULL, discard_chr = 11, MAF_filter = 0.05, MAF_col = "MAF", list_keep = NULL){
  ...
  ggplot2::ggplot(data, aes(x = Marker_Position_cum, y= .data[[y]])) +
    ggplot2::geom_point(aes(color = as.factor(.data[[chr_col]])), alpha = 0.8, size = 0.4) +
    ggplot2::scale_color_manual(values = rep(color_chr, length(levels(as.factor(data[[chr_col]]))))) +
